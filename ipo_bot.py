...
# NEW IMPORT: You need this library
...
# ... [Keep format_inr_number, money_inr, clean_ipo_name exactly as they were] ...
...
def format_inr_number(n, decimals=0):
    ...
    n = abs(n)
    s = f"{n:.{decimals}f}"
    if "." in s: int_part, frac = s.split(".")
    else: int_part, frac = s, ""
    if len(int_part) > 3:
        last3 = int_part[-3:]
        rest = int_part[:-3]
        groups = []
...
def clean_ipo_name(raw: str) -> str:
    ...
    if not s.upper().endswith("IPO"): s = s.rstrip() + " IPO"
    return s.strip()

# ---------- Scraper (Stealth Mode) ----------
async def scrape_live_ipos():
    async with async_playwright() as p:
        # LEVEL 2 HARDENING: Disable automation flags
        ...
        await stealth_async(page)  # This hides the 'selenium/webdriver' property

        # Mimic Google Referer
        ...
        try:
            ...
            print("📸 Saved error_screenshot.png and error_page.html")
            raise e
...
# ... [Keep filter_current_ipos, compose_telegram_message, main exactly as they were] ...
# (Just ensuring you keep the rest of your logic)
...
def compose_telegram_message(ipos):
    ...
    for ipo in ipos:
        ...
        if price and lot:
            ...
            shni_lots = 14
            if lot_cost * shni_lots > 200_000: shni_lots = 13
